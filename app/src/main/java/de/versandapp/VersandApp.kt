...
class VersandApp : Application() {
     ...
     * 2. Ship24-API – alle Dienste weltweit (inkl. China)
     * 3. Claude-Web-Recherche als Fallback ohne Ship24-Key
    ...
    private fun buildProviders(): List<TrackingProvider> = buildList {
        ...
        if (current.ship24ApiKey.isNotBlank()) {
            add(Ship24Provider(current.ship24ApiKey))
        }
